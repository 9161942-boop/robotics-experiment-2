...
\node[anchor=north west,xshift=22mm,yshift=-10mm,color=white,font=\sffamily\bfseries\small] at (current page.north west) {机器人学集成小组项目};
\node[anchor=north east,xshift=-30mm,yshift=-10mm,color=white,font=\sffamily\small] at (current page.north east) {第17组};
\end{tikzpicture}
\vspace*{13mm}
\begin{center}
{\color{Teal}\sffamily\bfseries\small 实验报告系列 \textbar\ ROS 2 + GAZEBO}\par
\vspace{9mm}
{\color{Navy}\fontsize{29}{34}\selectfont\sffamily\bfseries 实验二}\par
\vspace{3mm}
{\color{Blue}\fontsize{19}{24}\selectfont\sffamily 机械臂定点抓取与放置}\par
\vspace{6mm}
{\color{Teal}\rule{0.16\textwidth}{2pt}\hspace{5pt}\rule{0.52\textwidth}{0.6pt}\hspace{5pt}\rule{0.16\textwidth}{2pt}}\par
\vspace{5mm}
{\color{Ink}\large\sffamily 实验报告}\par
\vspace{8mm}
...
小组 & 第17组\\
组员 & 杨牧青；兰松语；张凯睿；赵航\\
平台 & ROS 2 Humble + Gazebo + mechArm 270\\
...
\vfill
\colorbox{SoftGray}{\parbox{0.78\textwidth}{\small\color{Ink}\textbf{内容概述。}本报告记录一个定点抓取与放置实验：机械臂从已知的抓取点 A 接近目标，完成夹持后抬升到安全高度，移动至放置点 B 释放目标，最后返回初始位姿。报告说明控制原理、ROS 2 实现、仿真验证结果和实机演示证据。}}\par
\vfill
{\color{Navy}\sffamily\bfseries 2026 年 9 月}\par
\end{center}
\end{titlepage}
