...
\node[anchor=north west,xshift=16mm,yshift=-18mm,color=white,font=\sffamily\bfseries\small] at (current page.north west) {机器人集成};
\node[anchor=north west,xshift=16mm,yshift=-25mm,color=white,font=\sffamily\small] at (current page.north west) {小组项目};
\node[anchor=south west,xshift=16mm,yshift=18mm,color=white,font=\sffamily\bfseries\small] at (current page.south west) {第 17 组  /  2026};
\end{tikzpicture}
\vspace*{18mm}
\noindent\begin{minipage}[t][0.83\textheight][t]{0.31\textwidth}
\color{white}\sffamily
{\color{Sky}\fontsize{11}{13}\selectfont\bfseries 实验 02}\par
\vspace{9mm}
{\fontsize{25}{29}\selectfont\bfseries 机械臂定点\par 抓取与放置}\par
\vspace{5mm}
{\color{Sky}\fontsize{13}{17}\selectfont mechArm 270}\par
\vspace{10mm}
\color{white}\rule{0.76\linewidth}{1.2pt}\par
\vspace{5mm}
{\fontsize{10}{14}\selectfont 实验报告}\par
\vfill
{\small\color{white}ROS 2 Humble\par Gazebo 仿真\par 实机演示}\par
\end{minipage}\hfill
\begin{minipage}[t][0.83\textheight][t]{0.58\textwidth}
\raggedright
{\color{Teal}\sffamily\bfseries\small 实验报告  |  ROS 2 + GAZEBO}\par
\vspace{4mm}
{\color{Navy}\sffamily\bfseries\large 六关节机械臂的可重复操作流程}\par
\vspace{5mm}
...
\vspace{3mm}
{\color{Ink}\footnotesize 手动示教控制台与 Gazebo 场景，用于确定定点并验证动作序列。}\par
\vspace{7mm}
...
小组 & 第 17 组\\
组员 & 杨牧青；兰松语；张凯睿；赵航\\
材料版本 & experiment2\_submission\_final\_20260913\_v2\\
仓库 & \url{github.com/9161942-boop/robotics-experiment-2}
\end{tabular}\vspace{3mm}}}\par
\vfill
{\color{Blue}\sffamily\bfseries 2026 年 9 月}\par
\end{minipage}
\end{titlepage}
...
\node[s](s1){手动控制台 / JSON\\示教记录};\node[s,right=of s1](s2){固化最终参数\\fixed\_point\_experiment.yaml};\node[s,right=of s2](s3){自动任务节点\\读取 YAML 序列};\node[t,right=of s3](s4){CSV、日志、视频\\与摘要};
\draw[a](s1)--(s2)--(s3)--(s4);
\end{tikzpicture}
\caption{从手动示教记录到 YAML 配置自动任务及证据生成的仿真流程。}\label{fig:simflowzh}
\end{figure}
\begin{figure}[H]\centering
...
\section{演示证据}
\subsection{仿真视频}
图~\ref{fig:simvideozh}从\code{simulation\_demo.webm}中选取三帧：第一帧显示机械臂接近工作区域，中间帧显示夹爪携带红色圆柱，最后一帧显示释放后机械臂回到场景初始侧。原始 WebM 随提交包归档。\begin{figure}[H]\centering\includegraphics[width=\textwidth]{sim_sequence.jpg}\caption{仿真演示视频中的代表性画面。}\label{fig:simvideozh}\end{figure}
\subsection{实机视频}
图~\ref{fig:realvideozh}概括了提供的竖屏 MP4，画面展示实体机械臂、夹爪、桌面和目标区域，记录了动作迁移到硬件后的运行过程。五周期数值结果见仿真 CSV 记录。\begin{figure}[H]\centering\includegraphics[width=\textwidth]{real_sequence.jpg}\caption{实机演示视频中的代表性画面。}\label{fig:realvideozh}\end{figure}
...
\begin{figure}[H]\centering\includegraphics[width=0.36\textwidth]{github_commit_history_exp2.png}\caption{公开实验二仓库的 GitHub 提交历史。}\label{fig:github-history-zh}\end{figure}
