...
\node[anchor=north west,xshift=22mm,yshift=-22mm,color=white,font=\sffamily\bfseries\small] at (current page.north west) {机器人集成小组项目};
\node[anchor=north west,xshift=22mm,yshift=-39mm,color=white,font=\sffamily\bfseries] at (current page.north west) {\fontsize{30}{34}\selectfont 实验二};
\node[anchor=north west,xshift=22mm,yshift=-61mm,color=white,font=\sffamily\bfseries] at (current page.north west) {\fontsize{22}{27}\selectfont 机械臂定点抓取与放置};
\node[anchor=south east,xshift=-22mm,yshift=18mm,color=white,font=\sffamily\bfseries\small] at (current page.north east) {第 17 组  /  2026 年 9 月};
\end{tikzpicture}
\vspace*{113mm}
\noindent{\color{Teal}\rule{0.14\textwidth}{2.2pt}}\par
\vspace{5mm}
{\color{Navy}\sffamily\bfseries\fontsize{23}{28}\selectfont mechArm 270 定点操作实验}\par
\vspace{3mm}
{\color{Ink}\sffamily\large ROS 2 Humble 与 Gazebo 实验报告}\par
\vspace{8mm}
...
\vfill
\noindent{\color{Blue}\sffamily\small 仿真验证  ·  ROS 2 控制  ·  实机演示}\par
\end{titlepage}
