...
\begin{enumerate}
\item \textbf{Build the Gazebo scene.} A \code{mecharm\_fixed\_pick\_sim} workspace was prepared under ROS 2 Humble and Gazebo Classic 11. The scene loaded the mechArm 270, parallel gripper, tabletop, target cylinder and placement area. Mass, inertia, collision bodies and friction coefficients were configured for the target so that contact, gripping, lifting and gravity release produced observable physical responses.
...
\item \textbf{Record waypoints and tune the parameters.} The panel recorded six joint angles, gripper position, contact-pad force and motion duration for each waypoint. The points \code{above\_pick}, \code{down\_pick}, \code{grasp\_object}, \code{lift\_object}, \code{move\_to\_place} and \code{release\_object} were added in execution order and saved to \code{manual\_animation.json}; the previous file was retained as \code{manual\_animation.json.bak}. Manual short cycles were used for parameter tuning: collision bodies, mass, inertia, friction and contact force were adjusted when the object was ejected or unstable; contact geometry and speed were adjusted when penetration or vibration occurred; B coordinates, release height, motion duration and gripper opening time were adjusted when the placement was offset.
\item \textbf{Launch the automatic simulation.} After the manually taught poses were stable, \code{run\_contact\_grasp.sh} launched the five-cycle task. The script cleaned stale Gazebo processes and environment variables, selected a dedicated Gazebo master URI, waited for controller initialization and passed the confirmed action sequence to the task node.
\item \textbf{Replay the sequence and check acceptance.} The automatic task node read the waypoints in \code{manual\_animation.json} and executed the \code{initial}, \code{above\_a}, \code{crab}, \code{pick}, \code{lift}, \code{release\_ready}, \code{release} and reset phases. Cosine interpolation generated 30~Hz joint commands. The program completed five cycles, checked the final X/Y/Z, XY error, height, joint limits, service responses and state transitions, and wrote the cycle number, phase, joint positions, gripper actions and checks to CSV. The terminal summary, two five-cycle CSV files, error log, checksums and simulation video were archived together.
\end{enumerate}
