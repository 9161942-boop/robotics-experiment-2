...
\node[anchor=north west,inner sep=0pt] at (current page.north west) {\includegraphics[width=\paperwidth,height=105mm]{figures/manual_control_console.png}};
\fill[Navy,opacity=0.80] (current page.north west) rectangle ([yshift=-105mm]current page.north east);
\fill[Teal] ([yshift=-105mm]current page.north west) rectangle ([yshift=-110mm]current page.north east);
\node[anchor=north west,xshift=22mm,yshift=-22mm,color=white,font=\sffamily\bfseries\small] at (current page.north west) {ROBOTICS INTEGRATION GROUP PROJECT};
\node[anchor=north west,xshift=22mm,yshift=-39mm,color=white,font=\sffamily\bfseries] at (current page.north west) {\fontsize{30}{34}\selectfont EXPERIMENT 02};
\node[anchor=north west,xshift=22mm,yshift=-61mm,color=white,font=\sffamily\bfseries] at (current page.north west) {\fontsize{22}{27}\selectfont FIXED-POINT PICK-AND-PLACE};
\node[anchor=south east,xshift=-22mm,yshift=18mm,color=white,font=\sffamily\bfseries\small] at (current page.north east) {GROUP 17  /  SEPTEMBER 2026};
\end{tikzpicture}
\vspace*{113mm}
\noindent{\color{Teal}\rule{0.14\textwidth}{2.2pt}}\par
\vspace{5mm}
{\color{Navy}\sffamily\bfseries\fontsize{23}{28}\selectfont Fixed-point manipulation with a mechArm 270}\par
\vspace{3mm}
{\color{Ink}\sffamily\large Laboratory report for ROS 2 Humble and Gazebo}\par
\vspace{8mm}
\colorbox{white}{\parbox{0.92\textwidth}{\vspace{4mm}
\begin{tabular}{>{\bfseries\color{Navy}\sffamily}p{0.21\textwidth}p{0.68\textwidth}}
...
\vfill
\noindent{\color{Blue}\sffamily\small Simulation verification  ·  ROS 2 control  ·  Real-arm demonstration}\par
\end{titlepage}
