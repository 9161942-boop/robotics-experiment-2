...
\fill[Teal] ([yshift=-7mm]current page.north west) rectangle ([yshift=-10mm]current page.north east);
\fill[Navy] (current page.south west) rectangle ([yshift=18mm]current page.south east);
\node[anchor=north west,xshift=18mm,yshift=-17mm,color=Navy,font=\sffamily\bfseries\small] at (current page.north west) {ROBOTICS INTEGRATION GROUP PROJECT};
\node[anchor=north east,xshift=-18mm,yshift=-17mm,color=Blue,font=\sffamily\bfseries\small] at (current page.north east) {GROUP 17};
\node[anchor=north east,xshift=-17mm,yshift=-42mm,color=Sky,font=\sffamily\bfseries] at (current page.north east) {\fontsize{120}{120}\selectfont 02};
\node[anchor=south west,xshift=18mm,yshift=6mm,color=white,font=\sffamily\small] at (current page.south west) {ROS 2 HUMBLE  ·  GAZEBO  ·  mechARM 270};
\node[anchor=south east,xshift=-18mm,yshift=6mm,color=white,font=\sffamily\small] at (current page.south east) {SEPTEMBER 2026};
\end{tikzpicture}
\vspace*{31mm}
\begin{center}
{\color{Teal}\sffamily\bfseries\small EXPERIMENT REPORT SERIES}\par
\vspace{8mm}
{\color{Navy}\sffamily\bfseries\fontsize{40}{46}\selectfont Experiment 2}\par
\vspace{4mm}
{\color{Blue}\sffamily\fontsize{20}{25}\selectfont Fixed-point Pick-and-place}\par
{\color{Ink}\sffamily\large with a mechArm 270}\par
\vspace{8mm}
{\color{Teal}\rule{0.12\textwidth}{2pt}\hspace{6pt}\rule{0.48\textwidth}{0.6pt}\hspace{6pt}\rule{0.12\textwidth}{2pt}}\par
\vspace{5mm}
{\color{Ink}\sffamily\large Laboratory report}\par
\end{center}
\vspace{18mm}
\noindent\begin{center}
\begin{tabular}{@{}>{\bfseries\color{Navy}\sffamily}l@{\hspace{7mm}}>{\sffamily}l@{}}
Course & Robotics Integration Group Project I\\[2mm]
Members & Yang Muqing; Lan Songyu; Zhang Kairui; Zhao Hang\\[2mm]
Evidence & experiment2\_submission\_final\_20260913\_v2\\[2mm]
Repository & \url{github.com/9161942-boop/robotics-experiment-2}
\end{tabular}
\end{center}
\vfill
\begin{center}{\color{Blue}\sffamily\small Simulation verification  ·  ROS 2 control  ·  Real-arm demonstration}\end{center}
\end{titlepage}
...
\end{enumerate}
\begin{figure}[H]\centering
\includegraphics[width=0.82\textwidth]{real_sequence.jpg}\par\vspace{2mm}
...
\end{tikzpicture}
\caption{Real-machine frames paired with the action chain from device preparation to grasp, transfer, release and reset.}\label{fig:realflow}
\end{figure}
...
Figure~\ref{fig:realflow} places representative hardware frames next to the action chain. The supplied portrait MP4 records the physical arm during approach, grasp, lift, transfer, release and reset. It is used as process evidence; the five-cycle numerical result remains supported by the simulation CSV records.
