...
\fill[Navy] (current page.north west) rectangle ([yshift=-27mm]current page.north east);
\fill[Teal] ([yshift=-27mm]current page.north west) rectangle ([yshift=-30mm]current page.north east);
\fill[Sky] ([xshift=-24mm]current page.north east) rectangle ([xshift=-15mm,yshift=-30mm]current page.north east);
\node[anchor=north west,xshift=22mm,yshift=-10mm,color=white,font=\sffamily\bfseries\small] at (current page.north west) {ROBOTICS INTEGRATION GROUP PROJECT};
\node[anchor=north east,xshift=-30mm,yshift=-10mm,color=white,font=\sffamily\small] at (current page.north east) {GROUP 17};
\end{tikzpicture}
\vspace*{13mm}
\begin{center}
{\color{Teal}\sffamily\bfseries\small EXPERIMENT REPORT SERIES \textbar\ ROS 2 + GAZEBO}\par
\vspace{9mm}
{\color{Navy}\fontsize{29}{34}\selectfont\sffamily\bfseries Experiment 2}\par
\vspace{3mm}
{\color{Blue}\fontsize{18}{23}\selectfont\sffamily Fixed-point Pick-and-place with a mechArm 270}\par
\vspace{6mm}
{\color{Teal}\rule{0.16\textwidth}{2pt}\hspace{5pt}\rule{0.52\textwidth}{0.6pt}\hspace{5pt}\rule{0.16\textwidth}{2pt}}\par
\vspace{5mm}
{\color{Ink}\large\sffamily Laboratory Report}\par
\vspace{8mm}
\colorbox{Sky}{\parbox{0.84\textwidth}{\vspace{4mm}
\begin{tabular}{>{\bfseries\color{Navy}\sffamily}p{0.24\textwidth}p{0.58\textwidth}}
...
\end{tabular}\vspace{4mm}}}\par
\vfill
\colorbox{SoftGray}{\parbox{0.78\textwidth}{\small\color{Ink}\textbf{Scope.} This report documents a fixed-point manipulation experiment: the arm approaches a known pick point A, grasps a target cylinder, carries it above a safe height, releases it at point B, and returns home. It records the control principle, ROS 2 implementation, simulation verification and real-machine demonstration evidence.}}\par
\vfill
{\color{Navy}\sffamily\bfseries September 2026}\par
\end{center}
\end{titlepage}
