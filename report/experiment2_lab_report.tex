...
\node[box] (s1) {Gazebo scene\\and controllers};\node[box,right=of s1] (s2) {Manual control panel\\teach poses};\node[box,right=of s2] (s3) {Record waypoints\\manual\_animation.json};
\node[box2,below=of s3] (s4) {Manual short cycle\\tune grasp/release};\node[box2,left=of s4] (s5) {Automatic task node\\replay sequence};\node[box2,left=of s5] (s6) {CSV, logs,\\video and summary};\draw[arr] (s1)--(s2)--(s3)--(s4)--(s5)--(s6);
\end{tikzpicture}
\caption{Simulation flow from manual teaching to automatic replay and evidence generation.}\label{fig:simflow}
\end{figure}
\begin{figure}[H]\centering
